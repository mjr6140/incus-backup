    ...
    cmd := &cobra.Command{
        ...
        RunE: func(cmd *cobra.Command, args []string) error {
            ...
            if len(args) == 0 {
                ...
                if err != nil { return fmt.Errorf("scan pools under %s: %w", base, err) }
                for _, p := range pools {
                    if !p.IsDir() || strings.HasPrefix(p.Name(), ".") { continue }
                    namesDir := filepath.Join(base, p.Name())
                    names, err := os.ReadDir(namesDir)
                    if err != nil { return err }
                    for _, n := range names { if n.IsDir() && !strings.HasPrefix(n.Name(), ".") { items = append(items, [2]string{p.Name(), n.Name()}) } }
                }
            } else {
                for _, a := range args {
                    var pool, name string
                    if parts := strings.SplitN(a, "/", 2); len(parts) == 2 { pool, name = parts[0], parts[1] }
                    if pool == "" || name == "" { return fmt.Errorf("invalid volume spec %q (expected POOL/NAME)", a) }
                    items = append(items, [2]string{pool, name})
                }
            }
            sort.Slice(items, func(i, j int) bool { if items[i][0]==items[j][0] { return items[i][1]<items[j][1] }; return items[i][0]<items[j][0] })
