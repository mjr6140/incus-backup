    ...
    cmd := &cobra.Command{
        ...
        RunE: func(cmd *cobra.Command, args []string) error {
            ...
            if len(args) == 0 {
                ...
                if err != nil { return err }
                for _, v := range vols { items = append(items, [2]string{v.Pool, v.Name}) }
            } else {
                for _, a := range args {
            ...
            for i, it := range items {
                ...
                if _, err := vol.BackupVolume(client, tgt.DirPath, project, pool, name, optimized, !noSnapshot, time.Now(), stdout); err != nil { return err }
