...
func BackupVolume(client incusapi.Client, root, project, pool, name string, optimized, snapshot bool, now time.Time, progressOut io.Writer) (string, error) {
    ...
    if snapshot {
        ...
        if progressOut != nil { fmt.Fprintf(progressOut, "[snapshot] create %s/%s@%s\n", pool, name, snapName) }
        if err := client.CreateVolumeSnapshot(project, pool, name, snapName); err != nil { return "", err }
        defer func() { if progressOut != nil { fmt.Fprintf(progressOut, "[snapshot] delete %s/%s@%s\n", pool, name, snapName) }; _ = client.DeleteVolumeSnapshot(project, pool, name, snapName) }()
    }
    ...
    if err := writeChecksums(snapDir, []string{"volume.tar.xz", "manifest.json"}); err != nil { return "", err }
    return snapDir, nil
}
