...
// When --skip-existing is used for volumes, restore should not modify the existing volume.
func TestVolumeRestore_SkipExisting(t *testing.T) {
    if os.Getenv("INCUS_TESTS") != "1" { t.Skip("INCUS_TESTS=1 not set") }

    proj := "itest-volskip-" + time.Now().UTC().Format("20060102T150405")
    ...
    // Attach and write original marker; detach before backup
    run(t, "incus", "--project", proj, "storage", "volume", "attach", pool, volName, inst, "/mnt/vol")
    run(t, "incus", "--project", proj, "exec", inst, "--", "sh", "-lc", "echo base > /mnt/vol/marker.txt")
    ...
    // Change content after backup
    run(t, "incus", "--project", proj, "storage", "volume", "attach", pool, volName, inst, "/mnt/vol")
    run(t, "incus", "--project", proj, "exec", inst, "--", "sh", "-lc", "echo changed > /mnt/vol/marker.txt")
    run(t, "incus", "--project", proj, "storage", "volume", "detach", pool, volName, inst)

    // Attempt restore with --skip-existing; should not modify the volume
    {
        var out, errb bytes.Buffer
        cmd := cli.NewRootCmd(&out, &errb)
        cmd.SetArgs([]string{"restore", "volume", pool+"/"+volName, "--project", proj, "--target", "dir:" + root, "--skip-existing"})
        if _, err := cmd.ExecuteC(); err != nil { t.Fatalf("restore volume --skip-existing: %v; stderr=%s", err, errb.String()) }
    }

    // Verify marker still shows "changed" (not overwritten)
    run(t, "incus", "--project", proj, "storage", "volume", "attach", pool, volName, inst, "/mnt/vol")
    out := run(t, "incus", "--project", proj, "exec", inst, "--", "sh", "-lc", "cat /mnt/vol/marker.txt || true")
    if !strings.Contains(out, "changed") { t.Fatalf("expected unchanged marker; got: %q", out) }
}
